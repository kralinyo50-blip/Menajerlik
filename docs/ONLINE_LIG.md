...
- Online ekran açıldığında oda önce tek bir istekle geri yüklenir, sonra canlı akışa geçilir. Akış hiç veri getirmezse (bazı ters proxy/tüneller SSE yanıtını tamponlar) ekran boş kalmaz: oyun otomatik olarak **yoklama** moduna geçer, tablo ve saha gecikmeli ama çalışır şekilde güncellenir ve birkaç dakika içinde akış yeniden denenir. Üstteki durum rozeti hangi yolda olduğunu gösterir ("Senkronize" / "Yoklamayla senkronize").
...
- **Canlı bağlantı ve kopmalar:** bağlantı canlı akışla (SSE) kurulur. Sekme yenilenirse aynı sekme kimliğiyle açılan yeni bağlantı eski (kopmuş görünen) bağlantıyı devralır; menajer başına en fazla **dört** canlı sekme açılır ve **en yeni bağlantı her zaman kazanır** — eski sekme akışı bırakıp birkaç saniyelik gecikmeli yoklamaya geçer, oyun hiçbir durumda "bağlanamıyor" ekranında kilitli kalmaz. Yoklamaya geçen sekme 45 saniyede bir akışı yeniden dener ve başarılıysa canlı karelere döner.
- Sunucu bir isteği **429** ile reddederse (dakikada 900'den fazla istek, menajer başına 30'dan fazla akış denemesi) yanıt `Retry-After` başlığı verir; istemci bu süreden önce yeniden denemez ve yoklama yedeğine geçebilir. Yazılanlar 30 saniye boyunca boşalmayan (okuyucusu kalmamış) akışlar sunucu tarafından düşürülür, toplam akış 2000 ile sınırlıdır.
...
Online testleri: kodla katılma, yetkilendirme ve anahtar gizliliği, isim/kapasite kontrolü, 18 haftalık çift devre fikstür, eşzamanlı hazır istekleri, eski hafta isteklerinin reddi, tüm sezon puan hesabı, sunucu yeniden başlatma, ayrılma, bot devri, yönetim devralma, girdi sınırları, origin kontrolü ve bozuk kayıt dosyası davranışı. Ek canlı testleri: birebir maçta aynı saha kareleri, yetkisiz takım komutları, tekrarlanan komutların tek uygulanması, oyuncu değişikliği sınırı, ortak duraklatma/devre arası, maç ortası yeniden başlatma ve kimliği doğrulanmış SSE akışlarının eşitliği. Bağlantı testleri: kopmuş (okunmayan) dört zombi soketin yeniden bağlanmayı kilitlememesi, aynı sekme kimliğiyle gelen istemcinin eski akışı devralması, akış açma hızı sınırının `Retry-After` ile bildirilmesi. `npm run test:modes` tarafındaki istemci testleri: sekme kimliğinin yenileme boyunca sabit kalması, SSE paket ayrıştırma (oda / devralındı / oturum bitti), yeniden deneme gecikmesinin üstel + jitterli olması ve `Retry-After` değerine saygı göstermesi.
