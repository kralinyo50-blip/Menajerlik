...
function run(command, params, options = {}) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(command, params, { cwd: ROOT, stdio: 'inherit', ...options });
...
export function spawnAndParse(command, params, parse, kind) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(command, params, { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] });
...
async function main() {
  ...
  if (args.build || !existsSync(resolve(ROOT, 'dist', 'index.html'))) {
    log('  [..] Oyun derleniyor (npm run build)…');
  ...
  if (!await waitForHealth(port)) {
    killTree(server);
    throw new Error('Sunucu saglik kontrolune yanit vermedi.');
  }
  log('  [OK] Sunucu acik.');
  log('  [..] Internet tüneli aciliyor (10-30 sn surebilir)…');

  const attempts = args.tunnel === 'cloudflared' ? ['cloudflared']
    : args.tunnel === 'localtunnel' ? ['localtunnel']
      : ['cloudflared', 'system', 'localtunnel'];
  let lastError;
  for (const kind of attempts) {
    try {
      if (kind === 'cloudflared') tunnel = await startCloudflaredTunnel(port, log);
      else if (kind === 'system') tunnel = await startSystemCloudflared(port, log);
      else tunnel = await startLocaltunnel(port, log);
      break;
    } catch (error) {
      lastError = error;
  ...
  if (!tunnel) {
    // Tünel açılamadıysa sunucuyu geride yetim bırakma.
    killTree(server);
    await new Promise(r => setTimeout(r, 500));
